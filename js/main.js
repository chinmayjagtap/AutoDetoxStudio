...
(() => {
  ...
  if (baSlider) {
    ...
    const start = (x) => { dragging = true; baSlider.classList.add('dragging'); setPos(x); };
    const move  = (x) => { if (dragging) setPos(x); };
    const end   = () => { dragging = false; baSlider.classList.remove('dragging'); };
  ...
  if (galleryWrap && galleryTrack) {
    ...
    const down = (x) => { isDown = true; galleryWrap.classList.add('dragging'); startX = x; scrollStart = galleryTrack.scrollOffset || 0; };
    let offset = 0;
    ...
    galleryWrap.addEventListener('mousedown', (e) => { isDown = true; galleryWrap.classList.add('dragging'); startX = e.clientX; scrollStart = offset; });
    window.addEventListener('mousemove', (e) => {
      if (!isDown) return;
      offset = Math.max(maxOffset(), Math.min(0, scrollStart + (e.clientX - startX)));
      applyOffset();
    });
    window.addEventListener('mouseup', () => { isDown = false; galleryWrap.classList.remove('dragging'); });

    galleryWrap.addEventListener('touchstart', (e) => { isDown = true; startX = e.touches[0].clientX; scrollStart = offset; }, { passive: true });
    ...
    $('#galleryNext')?.addEventListener('click', () => { offset = Math.max(maxOffset(), offset - step()); applyOffset(); });
    $('#galleryPrev')?.addEventListener('click', () => { offset = Math.min(0, offset + step()); applyOffset(); });
  }
  ...
  if (bookingForm) {
    ...
    /* Preselect package + jump to step 1 when clicked from Services / Packages sections */
  ...
  document.addEventListener('keydown', e => { if (e.key === 'Escape') { closeBooking(); $('#enquiryOverlay')?.classList.remove('show'); } });
  ...
  $('#galleryLightboxClose')?.addEventListener('click', () => { lightbox?.classList.remove('show'); lightbox?.setAttribute('aria-hidden','true'); });
  ...
  $('#galleryShuffle')?.addEventListener('click', () => {
    ...
    if (!track) return;
    [...galleryItems].sort(() => Math.random() - .5).forEach(item => track.appendChild(item));
  ...
  if (enquiryOverlay && !sessionStorage.getItem('ads_enquiry_seen')) {
    setTimeout(() => { openEnquiry(); sessionStorage.setItem('ads_enquiry_seen','1'); }, 9000);
  }
